...
feedbackHtml = fmap (headerHtml <>) . renderToHtml . pretty
  ...
  where
    ...
    headerHtml = (<> "\n") $ Html.style $ Html.toHtml $ unlines
      ...
      , "    width: 10px;"
      , "    height: 35px;"
      , "    clip-path: polygon(0 5px, 5px 0px, 10px 5px, 10px 30px, 5px 35px, 0px 30px, 0 5px);"
      , "    margin: 0;"
      , "    padding: 0;"
      , "  }"
      ...
      , "    margin: 0;"
      , "  }"
      , "  .syrup-segmentA, .syrup-segmentD, .syrup-segmentG { top: 0 }"
      , "  .syrup-segmentC, .syrup-segmentF { left: 40px;  }"
      , ""
      ]
