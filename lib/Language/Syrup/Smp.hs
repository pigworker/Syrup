...
xor = let ty = Bit Unit in
  Def ("xor", PVar ty <$> ["X", "Y"]) (App (ty :| []) "or" (Var ty <$> ["A", "B"]) :| []) $ Right
...
tff = let ty = Bit Unit in
  Def ("tff", [PVar ty "T"]) (Var ty "Q" :| []) $ Right $
...
foo = let ty = Bit Unit in
  Def ("foo", PVar ty <$> ["A", "B", "C"])
      (App (ty :| []) "and" [Var ty "A", Var ty "B"] :| [Var ty "Z"]) $ Right $
      (((PVar ty "Z" :| [])
        :=: ((App (ty :| []) "or" [Var ty "A", App (ty :| []) "and" [Var ty "B", Var ty "C"]])
            :| []))
      :| [])
...
swapG = let ty = Bit Unit in
  Def ("swap", [PVar ty "x", PVar ty "y"]) (Var ty "y" :| [Var ty "x"]) (Left False)
...
rca4 = let ty = Bit Unit in
  Def ("rca4", PVar ty <$> ["X3", "X2", "X1", "X0", "Y3", "Y2", "Y1", "Y0", "CI"])
      (Var ty "CO" :| (Var ty <$> ["Z3", "Z2", "Z1", "Z0"])) $ Right
...
andnot = let ty = Bit Unit in
  Def ("andnot", [PVar ty "X"]) (Var ty "R" :| []) $ Right
